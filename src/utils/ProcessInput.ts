import { ValidConstants, ValidVariables } from "../types/types";
import validateChars from "./ValidateChars";
import validateSyntax from "./ValidateSynntax";
...
let ProccessInput = (input: string[][]) => {
  ...
  for (let i = 0; i < input.length; i++) {
    if (input[i].includes("=")) {
      ...
      if (
        !ValidConstants.includes(input[i][0]) &&
        !ValidVariables.includes(input[i][0])
      ) {
